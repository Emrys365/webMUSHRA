function TaggingDemoPage(_pageManager, _pageTemplateRenderer, _audioContext, _bufferSize, _audioFileLoader, _session, _tagDataSender, _pageConfig, _mushraValidator, _errorHandler, _language, _pageCount) {
  // Only for demonstration of tagged audios (cannot edit the tagging results)
...
TaggingDemoPage.prototype.getName = function () {
  return this.pageConfig.name;
};

TaggingDemoPage.prototype.loadAudiosFromTsv = function (_file) {
  ...
  for (var i = 0; i < data.length; i++) {
    const uid = data[i][uidIndex], path = escapeUrl(data[i][pathIndex]);
  ...
  if (tagsIndex == -1) {
    this.showToast("No tags found for the audios.", 3, true, "./design/images/warning.svg");
  } else {
    for (var i = 0; i < data.length; i++) {
      const uid = data[i][uidIndex];
      var tags = data[i][tagsIndex];
      if (tags != null && tags.length > 0) {
        tags = tags.split(";");
        if (!this.validateLabels(new Set(tags))) {
          this.showError("Invalid label found in TSV (" + _file + ") for " + uid + ": " + data[i][tagsIndex]);
          return;
        }
        this.labels[uid] = new Set(tags);
      }
    }
  }
};

TaggingDemoPage.prototype.init = function () {
...
TaggingDemoPage.prototype.render = function (_parent) {
  var div = $("<div></div>");
  _parent.append(div);
...
TaggingDemoPage.prototype.renderMetaInfo = function () {
  var tableMeta;
...
TaggingDemoPage.prototype.renderTags = function () {
...
TaggingDemoPage.prototype.updateLabel = function (_uid) {
...
TaggingDemoPage.prototype.prevAudio = function (event) {
  if (this.currentIndex > 0) this.jumpTo(this.currentIndex - 1);
};

TaggingDemoPage.prototype.nextAudio = function (event) {
  if (this.currentIndex < this.lastIndex) this.jumpTo(this.currentIndex + 1);
};

TaggingDemoPage.prototype.jumpTo = function (_index) {
...
TaggingDemoPage.prototype.pause = function() {
    this.tagAudioControl.pause();
};

TaggingDemoPage.prototype.setLoopStart = function() {
...
TaggingDemoPage.prototype.setLoopEnd = function() {
  var slider = document.getElementById('slider');
...
TaggingDemoPage.prototype.btnCallback = function() {
...
TaggingDemoPage.prototype.loadAudioFiles = function (_index) {
...
TaggingDemoPage.prototype.load = function () {
...
TaggingDemoPage.prototype.popupShortcuts = function () {
...
TaggingDemoPage.prototype.popupSettings = function () {
  if ($("#settingsPopupCard-popup").hasClass("ui-popup-active")) {
...
TaggingDemoPage.prototype.showToast = function (_msg, _duration, _close, _avatar) {
...
TaggingDemoPage.prototype.showError = function (_msg) {
 ...
 */
TaggingDemoPage.prototype.validateLabels = function (_labels) {
...
TaggingDemoPage.prototype.save = function () {
  this.tacic.unbind();
  Mousetrap.unbind(['[', '【', ']', '】', '/']);
...
TaggingDemoPage.prototype.store = function () {
};
