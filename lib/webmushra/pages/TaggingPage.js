...
TaggingPage.prototype.render = function (_parent) {
  ...
  Mousetrap.bind(['[', '【'], function() {
    ...
    if ($("#tabPrev").hasClass("ui-disabled")) return false;
    $("#tabPrev").click();
    return false;
  });
  Mousetrap.bind([']', '】'], function() {
    if (this.disableShortcuts === true) return false;
    if ($("#tabNext").hasClass("ui-disabled")) return false;
    $("#tabNext").click();
    return false;
  });
...
TaggingPage.prototype.renderMetaInfo = function () {
  ...
  for (var name in this.metaInfo[this.currentIndex]) {
    ...
    var tdMetaInfo = $("<td><span class='metaInfo'>" + this.metaInfo[this.currentIndex][name] + "</span></td>");
...
TaggingPage.prototype.loadLabelCache = function (_overwrite) {
  ...
  if (this.labelCache === true) {
    ...
    if (count > 0) {
      this.showToast("Label cache of " + count + " audio(s) loaded from previous sessions. If you haven't participated in this session before, please clear the cache (by clicking the [Settings] -> [Delete] button on the upper-right corner) and then refreshing the page.", -1, true, "./design/images/warning.svg");
    }
  }
};
