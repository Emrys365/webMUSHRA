    ...
    if (this.pageConfig.showResults) {
      ...
      for (i = 0; i < trials.length; ++i) {
        ...
        if (trial.type === "wave_spec_tagging") {
	        ...
	        thT = $("<th colspan='2'></th>");
	        $(thT).append(trial.id + " (Tagging)" );
	        $(trT).append(thT);
	        $(table).append(trT);

	        var responses = trial.responses;
	        for (var j = 0; j < Math.min(responses.length, MAXITEMS); ++j) {
	          trTaggings = document.createElement("tr");
	          tdTaggingStimulus = document.createElement("td");
	          tdTaggingLabel = document.createElement("td");

	          tdTaggingStimulus.width = "30%";
	          tdTaggingLabel.width = "70%";

	          var rating = responses[j];

	          $(tdTaggingStimulus).append(rating.stimulus + ": ");
	          $(tdTaggingLabel).append(rating.labels);
	          $(trTaggings).append(tdTaggingStimulus);
	          $(trTaggings).append(tdTaggingLabel);
	          $(table).append(trTaggings);
	        }
          if (responses.length > MAXITEMS) {
            ...
            tdTaggingStimulus.width = "30%";
	          tdTaggingLabel.width = "70%";
            $(tdTaggingStimulus).append("...");
            $(tdTaggingLabel).append("...");
            $(trTaggings).append(tdTaggingStimulus);
            $(trTaggings).append(tdTaggingLabel);
            $(table).append(trTaggings);
          }
	     ...
	     } else if (trial.type === "mushra" || trial.type === "multi_metric_mushra") {
	        trT = document.createElement("tr");
	        thT = $("<th colspan='2'></th>");
