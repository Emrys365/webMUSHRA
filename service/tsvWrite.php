...
array_push($taggingTsvData, $input);
...
foreach ($session->trials as $trial) {
  if ($trial->type == "wave_spec_tagging") {
	...
	foreach ($trial->responses as $response) {
		...
		array_push($taggingTsvData, $results);
	}
  }
}
...
array_push($mushraTsvData, $input);
	...
	$write_mushra = true;
	  ...
	  foreach ($trial->responses as $response) {
		...
		array_push($results, $trial->id, $response->stimulus, $response->score, $response->time, $response->comment);
	  	...
	  	array_push($mushraTsvData, $results);


	  }
	    /*array_push($mushraTsvData, array($session->testId, $session->participant->email, $session->participant->age, $session->participant->gender, $trial->id, $response->stimulus, $response->score, $response->time, $response->comment));
		 *
		 */
  }
}
...
$write_pc = false;
$pcTsvData = array();
// array_push($pcTsvData, array("session_test_id", "participant_email", "participant_age", "participant_gender", "trial_id", "choice_reference", "choice_non_reference", "choice_answer", "choice_time", "choice_comment"));
  ...
  if ($trial->type == "paired_comparison") {
	  foreach ($trial->responses as $response) {
		...
		array_push($results, $trial->id, $response->reference, $response->nonReference, $response->answer, $response->time, $response->comment);
	  	...
	  	array_push($pcTsvData, $results);


	    // array_push($pcTsvData, array($session->testId, $session->participant->email, $session->participant->age, $session->participant->gender, $trial->id, $response->reference, $response->nonReference, $response->answer, $response->time, $response->comment));
	  }
  }
}
...
array_push($bs1116TsvData, $input);

// array_push($bs1116TsvData, array("session_test_id", "participant_email", "participant_age", "participant_gender", "trial_id", "rating_reference", "rating_non_reference", "rating_reference_score", "rating_non_reference_score", "rating_time", "choice_comment"));
foreach ($session->trials as $trial) {
  if ($trial->type == "bs1116") {
	  foreach ($trial->responses as $response) {
		...
		array_push($results, $trial->id, $response->reference, $response->nonReference, $response->referenceScore, $response->nonReferenceScore, $response->time, $response->comment);
	  	...
	  	array_push($bs1116TsvData, $results);

	    // array_push($bs1116TsvData, array($session->testId, $session->participant->email, $session->participant->age, $session->participant->gender, $trial->id, $response->reference, $response->nonReference, $response->referenceScore, $response->nonReferenceScore, $response->time, $response->comment));
	  }
  }
}
