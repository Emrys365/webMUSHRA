...
def make_enh_page(idx, total_idx, ref_wav_path, test_wav_dict):
    ...
    return {
        ...
        "type": "mushra",
        "id": wav_id,
        "content": "Listen to a reference audio and several processed audios. "
...
def main():
    ...
    for subset in args.test_root_wav_dir:
        test_sets[Path(subset).name] = {
            Path(p).stem: p for p in sorted(find_files(subset))
        }
